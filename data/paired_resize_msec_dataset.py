...
class PairedOneToManyBilateralDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(PairedOneToManyBilateralDataset, self).__init__()
        self.opt = opt
        ...
        for lq_path in lq_paths:
            id = lq_path[0:5]
            gt_path = [gt_path for gt_path in gt_paths if gt_path[:5] == id][0]
            lq_path = os.path.join(self.lq_folder, lq_path)
            gt_path = os.path.join(self.gt_folder, gt_path)
            self.paths.append(dict([('lq_path', lq_path), ('gt_path', gt_path)]))
